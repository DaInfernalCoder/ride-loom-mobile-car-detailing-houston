...
class SiteParser(HTMLParser):
    def __init__(self) -> None:
        ...
        self.email_links = 0
        self.phone_links = 0
        self.pending_phone_links = 0
        self.booking_links = 0
    ...
    def handle_starttag(self, tag: str, attrs: list[tuple[str, str | None]]) -> None:
        ...
        if href.startswith("tel:+"):
            self.phone_links += 1
        if href == "#contact" and data.get("aria-disabled") == "true":
            self.pending_phone_links += 1
        if href == "https://cal.com/sumitdatta/auto-detail-service":
            self.booking_links += 1
...
def main() -> int:
    ...
    if scan.forms:
        errors.append(f"public forms found: {scan.forms}")
    if scan.email_links:
        errors.append(f"public email links found: {scan.email_links}")
    if scan.booking_links < 6:
        errors.append(
            f"expected at least six booking paths, found {scan.booking_links}"
        )
    if scan.phone_links < 6 and scan.pending_phone_links < 6:
        errors.append(
            "expected at least six phone links or six explicitly disabled phone-pending controls, "
            f"found {scan.phone_links} phone and {scan.pending_phone_links} pending"
        )
    ...
    forbidden = {
        ...
        "five-star": "unverified review claim",
        "guarantee": "unverified guarantee claim",
    }
    lowered = text.lower()
    ...
    print(
        f"h1: {scan.h1_count}; local refs: {len(scan.local_refs)}; "
        f"anchors: {len(scan.hash_links)}; phone: {scan.phone_links}; "
        f"pending phone: {scan.pending_phone_links}; booking: {scan.booking_links}"
    )
